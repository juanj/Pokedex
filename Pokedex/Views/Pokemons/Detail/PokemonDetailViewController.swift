...
class PokemonDetailViewController: UIViewController {
    ...
    private func registerCells() {
        ...
        tableView.register(UINib(nibName: "PokemonCaptureTableViewCell", bundle: nil), forCellReuseIdentifier: Constants.CellIds.pokemonCaptureCell)
        tableView.register(UINib(nibName: "PokemonSpritesTableViewCell", bundle: nil), forCellReuseIdentifier: Constants.CellIds.pokemonSpritesCell)
    }
            ...
            case .stats:
                ...
                case .abilities:
                    if indexPath.row == 0 {
                        ...
                        guard let cell = tableView.dequeueReusableCell(withIdentifier: Constants.CellIds.pokemonAbilityCell) as? PokemonAbilityTableViewCell else {
                            fatalError("Cannor dequeue cell with reusable identifier \(Constants.CellIds.pokemonAbilityCell)")
                        }
                        cell.load(viewModel: viewModel.abilities[indexPath.row - 1])
                        return cell
                    }
                case .breeding:
                    if indexPath.row == 0 {
                ...
                case .capture:
                    if indexPath.row == 0 {
                        ...
                        guard let cell = tableView.dequeueReusableCell(withIdentifier: Constants.CellIds.pokemonCaptureCell) as? PokemonCaptureTableViewCell else {
                            fatalError("Cannor dequeue cell with reusable identifier \(Constants.CellIds.pokemonCaptureCell)")
                        }
                        if let viewModel = viewModel.capture {
                            cell.load(viewModel: viewModel)
                        }
                        return cell
                    }
                case .sprites:
                    if indexPath.row == 0 {
                        ...
                        guard let cell = tableView.dequeueReusableCell(withIdentifier: Constants.CellIds.pokemonSpritesCell) as? PokemonSpritesTableViewCell else {
                            fatalError("Cannor dequeue cell with reusable identifier \(Constants.CellIds.pokemonSpritesCell)")
                        }
                        cell.load(viewModel: viewModel.sprites)
                        return cell
                    }
                }
            case .evolutions:
